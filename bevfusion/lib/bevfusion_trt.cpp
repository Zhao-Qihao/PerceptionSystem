...
bool BEVFusionTRT::detect(
  ...
  const std::vector<sensor_msgs::Image::ConstPtr> & image_msgs,
  ...
  std::vector<Box3D> & det_boxes3d, std::unordered_map<std::string, double> & proc_timing)
{
  ...
  if (!preProcess(pc_msg, image_msgs, camera_masks, tf_buffer)) {
    ROS_ERROR("Pre-process failed. Skipping detection.");
    return false;
  }

  if (!inference()) {
    ROS_ERROR("Inference failed. Skipping detection.");
    return false;
  }
  // proc_timing.emplace(
  ...
  if (!postProcess(det_boxes3d)) {
    ROS_ERROR("Post-process failed. Skipping detection");
    return false;
  }
  // proc_timing.emplace(
...
void BEVFusionTRT::setIntrinsicsExtrinsics(
  std::vector<sensor_msgs::CameraInfo> & camera_info_vector,
...
bool BEVFusionTRT::inference()
{
  ...
  if (!status) {
    ROS_ERROR("Fail to enqueue and skip to detect.");
    return false;
  }
