...
CenterPoint::CenterPoint(std::string packagePath,std::string modelFile, bool verbose): verbose_(verbose)
{
    ...
    std::string scn_path = packagePath + "/model/centerpoint.scn.onnx";
...
int CenterPoint::doinfer(void* points, unsigned int point_num, cudaStream_t stream)
{
    ...
    if (verbose_) {
        ...
        // for (size_t i = 0; i < nms_pred_.size(); ++i) {
        //     const Bndbox& box = nms_pred_[i];
        //     if (box.score > 0.4) {
        ...
        //               << "score=" << box.score << ", "
        //               << "class_id=" << box.id << ", "
        //               << "label=" << label_map_[box.id] << std::endl;
        //     }
        // }
    }
    return 0;
