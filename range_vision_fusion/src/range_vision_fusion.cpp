  ...
  const cv::Mat &image_)
{
  ...
  // TODO: publish range_in_cv to debug
  publisher_range_in_cv_.publish(range_in_cv); // 发布转换后的激光雷达检测结果
  ...
  cv::Mat image_with_rect = image_.clone();

  std::vector<std::vector<size_t> > vision_range_assignments(in_vision_detections->objects.size()); // 记录每个视觉检测与哪些激光雷达检测重叠。
  std::vector<bool> used_vision_detections(in_vision_detections->objects.size(), false); // 记录某个视觉检测是否已被匹配。
  std::vector<long> vision_range_closest(in_vision_detections->objects.size());  // 记录每个视觉检测对应的最近激光雷达检测索引。
  ...
  for (size_t i = 0; i < in_vision_detections->objects.size(); i++)
  {
    ...
    for (size_t j = 0; j < range_in_cv.objects.size(); j++)
    {
      ...
      // TODO: visualize range_rect
      // Visualize range_rect on the image
      if (!image_.empty()) // Ensure the image is not empty
      { 
        // Define the rectangle properties
        cv::Scalar rect_color(0, 255, 0); // Green color (BGR format)
        int rect_thickness = 2;           // Thickness of the rectangle border

        // Draw the rectangle on the image
        cv::rectangle(image_with_rect, range_rect, rect_color, rect_thickness);
      }
    ...
    // 将处理后的图像转换回ROS图像消息
    out_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", image_with_rect).toImageMsg();
    out_msg->header = in_vision_detections->header;
    // 发布带有目标框的图像
    pub_image_.publish(out_msg);
  }

  std::vector<bool> used_range_detections(range_in_cv.objects.size(), false); // 标记哪些LiDAR检测已被匹配并加入融合结果。
...
ROSRangeVisionFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
  ...
  if (min_person_dimensions.size() == 3)
  {
  ...
  if (min_truck_dimensions.size() == 3)
  {
  ...
  else
  {
    ...
    detections_synchronizer_->registerCallback(
      boost::bind(&ROSRangeVisionFusionApp::SyncedDetectionsCallback, this, _1, _2, _3));
  }
  ...
  ROS_INFO("[%s] Publishing range_in_cv on topic: %s", __APP_NAME__, "/detection/range_in_cv");
