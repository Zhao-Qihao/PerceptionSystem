...
private:
  ...
  bool remove_behind_lane_;
  double keep_behind_lane_distance_;
  ...
  bool IsNormalObject(const autoware_msgs::DetectedObject &detected_object);
  ...
  void keepLanePoints(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
                      ...
                      bool remove_behind_lane = true,
                      float keep_behind_lane_distance = 10);

  void KeepInBehindLinePoints(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
                           pcl::PointCloud<pcl::PointXYZ>::Ptr out_cloud_ptr,
                           float in_distance = 10);
...
EuclideanClusterDetector::EuclideanClusterDetector()
  ...
  nh_private_.param("remove_behind_lane", remove_behind_lane_, true);
  ROS_INFO("[%s] remove_behind_lane: %d", APP_NAME, remove_behind_lane_);
  nh_private_.param("keep_behind_lane_distance", keep_behind_lane_distance_, 10.0);
  ROS_INFO("[%s] keep_behind_lane_distance: %f", APP_NAME,
           keep_behind_lane_distance_);
...
void EuclideanClusterDetector::rawPointCloudCallback(
  ...
  if (!using_sensor_cloud_) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr inlanes_cloud_ptr(
        new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr in_behind_lane_cloud_ptr(
        new pcl::PointCloud<pcl::PointXYZ>);
...
bool EuclideanClusterDetector::IsNormalObject(const autoware_msgs::DetectedObject &in_object)
{
  if (in_object.dimensions.x * in_object.dimensions.y > 26) // 20 是检测到的最大的物体，卡车最大为2.5*8
  {
    return false;
  }
  else if (in_object.dimensions.x > 9 || in_object.dimensions.y > 9)
  {
    return false;
  }
  else if ((in_object.dimensions.x > 8 || in_object.dimensions.y > 8) && in_object.dimensions.z < 1)
  {
    return false;
  }
  else if (in_object.dimensions.z < 0.5 && ((in_object.dimensions.x / in_object.dimensions.y > 4) || (in_object.dimensions.y / in_object.dimensions.x > 4)))
  {
    return false;
  }
  // else if (in_object.dimensions.x * in_object.dimensions.y*in_object.dimensions.z<1)
  // {
  //   return false;
  // }
  return true;
...
void EuclideanClusterDetector::publishDetectedObjects(
  ...
  for (size_t i = 0; i < in_clusters.clusters.size(); i++) {
    ...
    if (IsNormalObject(detected_object))
    {
...
void EuclideanClusterDetector::keepLanePoints(
    ...
    bool remove_behind_lane, float in_behind_lane_distance) {
  ...
  for (unsigned int i = 0; i < in_cloud_ptr->points.size(); i++) {
    ...
    if (remove_behind_lane) {
      if (current_point.x < -1.0 * in_behind_lane_distance) {
        far_indices->indices.push_back(i);
      }
    }
  }
